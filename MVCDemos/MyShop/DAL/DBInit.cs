// DB Seeding - Typically used for testing the web application. 
...
public static class DBInit
{
  ...
  {
    // Create a new scope to obtain a new ItemDbContext instance
    // A scope is a way to manage the lifetime of an object in an ASP.NET Core application
    using var serviceScope = app.ApplicationServices.CreateScope();
    ItemDbContext context = serviceScope.ServiceProvider.GetRequiredService<ItemDbContext>();
    context.Database.EnsureDeleted(); // Deletes the database if it already exists
    ...
    if (!context.Customers.Any())
    {
      var customers = new List<Customer>
      {
        ...
        new Customer { Name = "Bob Jensen", Address = "Bergenstien 2"},
      };
    ...
    if (!context.OrderItems.Any())
    {
      ...
      foreach (var orderItem in orderItems)
      {
        ...
        // item?.Price ?? 0 in other words: if item is null, return 0, otherwise return item.Price
      }
